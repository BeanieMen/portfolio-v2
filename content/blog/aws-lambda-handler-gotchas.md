...
titleMarkdown: >-
    ...
    <span style="color:#00a3e0">Lambda</span>
    <span style="color:#eaf4ef">InvalidEntrypoint</span>
    <span style="color:#74c7ec">Debugging</span>
date: "2025-11-28"
excerpt: "Runtime.InvalidEntrypoint can hide async handler issues, architecture mismatches, and file permission bugs."
tags:
  - aws
  - lambda
  - python
...
If you are staring at `Runtime.InvalidEntrypoint` in CloudWatch, your Docker image looks correct, and `python3 -c "import handler"` works inside the container, there are three common causes that all produce the same useless error.

## Async handlers are not awaited

Lambda does not natively support async handlers. It calls your function, gets a coroutine object, and dies trying to serialize it.
...
Lambda runs non-root. Files copied with restrictive permissions cannot be read:
